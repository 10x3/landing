  ...
  return (
    <>
      ...
      <CTABand
        ...
        sub="Book a 30-min call. We'll walk through your project, tell you who'd be on it, and - if we're a fit - lock you in at 50% off for 3 months."
      ...
      <Container className="relative">
        <div className="max-w-3xl">
          ...
          <h1 className="mt-6 font-sans text-5xl md:text-7xl font-semibold text-zinc-900 tracking-[-0.04em] leading-[1.02]">
            {hero.headline}
            <br />
            <span className="text-brand-600">{hero.headlineSecondLine}</span>
          </h1>
      ...
      <div className="mt-14 divide-y divide-hairline border-y border-hairline">
        {networkPrinciples.map((p, i) => (
          <div key={p.title} className="py-8 grid md:grid-cols-12 gap-6">
            <div className="md:col-span-1 font-mono text-xs text-[#d99a2b] tracking-[0.18em]">
              {String(i + 1).padStart(2, "0")}
            </div>
            <div className="md:col-span-4 font-sans text-2xl md:text-3xl font-medium tracking-[-0.02em] text-zinc-900 leading-snug">
              {p.title}
            </div>
            <div className="md:col-span-7 text-zinc-500 leading-relaxed md:pt-1">{p.body}</div>
          </div>
        ))}
      </div>
    </Section>
    ...
    <Section tone="surface">
      ...
      <ul className="mt-14 grid sm:grid-cols-2 md:grid-cols-3 gap-0 border-t border-l border-hairline">
        {vettingBar.map((item) => (
          <li
            key={item}
            className="p-6 border-r border-b border-hairline text-sm text-zinc-800 flex gap-3 items-start bg-zinc-50"
          >
      ...
      <ul className="mt-14 divide-y divide-hairline border-y border-hairline">
        {manifesto.map((m, i) => (
          <li key={m} className="py-6 flex gap-8 items-baseline">
            <span className="font-mono text-xs text-[#d99a2b] tracking-[0.18em] uppercase w-10 flex-shrink-0">
              {String(i + 1).padStart(2, "0")}
            </span>
            <span className="font-sans text-xl md:text-2xl text-zinc-900 tracking-[-0.015em] leading-snug">
              {m}
            </span>
          </li>
        ))}
        ...
        <ul className="mt-10 space-y-5">
          {disqualifier.points.map((p) => (
            <li key={p} className="flex gap-5 items-baseline">
              <span className="text-[#d99a2b] font-mono">{"-"}</span>
              <span className="text-xl text-zinc-800 leading-relaxed tracking-[-0.01em]">{p}</span>
            </li>
          ))}
        </ul>
      </div>
